...
function Salt() {
  ...
  const waterChangePercent = Math.round((waterChangeAmount / totalVolume) * 100);

  const amountOfSalt = (totalVolume * (disiredConcentration - currentConcentration)).toFixed(2); 
           ...
           <div className="salt_form-wrapper">
            ...
            <div className="salt_left_form">
              <Form layout="vertical">
                ...
                <Form.Item label={`Current concentration: ${currentConcentration}%`} className="salt_current">
                  <Slider
                    min={0}
                    max={2}
                    step={0.01}
                ...
                <Form.Item label={`Disired concentration: ${disiredConcentration}%`} className="salt_disired">
                  <Slider
                    min={0}
                    max={2}
                    step={0.01}
                ...
                <Form.Item label={`Water change: ${waterChangeAmount} l (${waterChangePercent}%)`}>
                  <Slider
                    min={0}
                    max={totalVolume}
                    step={8}
            ...
            <div className="salt_right_form">
               <div className="salt_disclaimer">
                 <h3>Disclaimer</h3>
                 <p>Only add pure salt without iodine to the water! Too high of a
                   salt concentration can be damaging to the koi. If you are unsure, 
                   please talk to a veterinarian. For fighting diseases and algae or for
                   taking precautions a concentration of 0.5 % is recommended.</p>
               </div>       
            </div>
           </div> 
