...
import { Form, Input, Button, Modal, Upload, Row, Col, notification } from "antd";
...
function Environment() {
    ...
    const handleDeletePond = async (index) => {
        ...
        try {
            const response = await fetch(`https://koicaresystemapi.azurewebsites.net/api/Pond/Delete-Pond/${pondId}`, {
                method: 'DELETE',
                headers: {
            ...
            notification.error({
                ...
                description: 'There was an issue deleting the pond. Please try again later.',
                placement: 'topRight',
            });
        }
    };
    ...
    const handleOk = async () => {
        ...
        try {
            ...
            if (isEditing) {
                response = await fetch(`https://koicaresystemapi.azurewebsites.net/api/Pond/Update-Pond/${pondDataList[editingIndex].pondId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                response = await fetch(`https://koicaresystemapi.azurewebsites.net/api/Pond/Create-Pond?accId=${user.accId}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    return (
        <div className='EnvironmentPage'>
            ...
            <div className='Environment__body'>
                ...
                <div className='add'>
                    <Button type="secondary" onClick={showModalCreate}>Add Pond</Button>
                </div>
            ...
            {pondDataList.length > 0 ? (
                            pondDataList.map((pondData, index) => (
                                <div key={index} className='uploaded__pond__info'>
                                    <h3>Pond {pondData.name ? pondData.name.toUpperCase() : 'No Name Available'}</h3>
                                    <Row gutter={16}>
                                        ...
                                        <Col md={12} xs={24} className='uploaded__pond__info__right'>
                                            <Row gutter={16}>
                                                <Col span={12} className='pond__infor'>
                                                    <p><strong>Pumping Capacity:</strong> {pondData.pumpCapacity} L/min</p>
                                                    <p><strong>Drain Count:</strong> {pondData.drainCount}</p>
                                                </Col>
                                                <Col span={12} className='pond__infor'>
                                                    <p><strong>Depth:</strong> {pondData.depth} m</p>
                                                    <p><strong>Volume:</strong> {pondData.volume} m³</p>
                                                </Col>
                                            </Row>
